...
async function resendCode() {
  loading.value = true
  try {
    const response = await api.post('/card-users/forgot-password', { email: email.value })
    if (response.success) {
...
async function onSubmit() {
  ...
  try {
    const response = await api.post('/card-users/verify-reset-code', {
      email: email.value,
      code: code.value
    })
    ...
    if (response.success) {
      ...
      sessionStorage.setItem('verified_reset_code', code.value)
